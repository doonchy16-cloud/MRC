...
internal static class Program
{
    ...
    public static int Main()
    {
        var tests = new (string Name, Action Body)[]
        {
            ...
            ("packaging creates the v0.0.15 pre-cert zip checksum and icon authority", PackagingSkeleton)
        };
    ...
    private static void VersionAliases()
    {
        ...
        foreach (var alias in new[] { "-v", "-version", "--version" })
        {
            ...
            Require(result.Output.Contains("Version: 0.0.15", StringComparison.Ordinal), $"{alias} omitted v0.0.15 identity.");
    ...
    private static void PackagingSkeleton()
    {
        ...
        var zipPath = Path.Combine(RepoRoot, "artifacts", "MRC-v0.0.15-win-x64.zip");
        var sumsPath = Path.Combine(RepoRoot, "artifacts", "SHA256SUMS.txt");
        Require(File.Exists(zipPath), "v0.0.15 candidate ZIP was not produced."); Require(File.Exists(sumsPath), "SHA256SUMS.txt was not produced.");
        ...
        Require(checksumText.Contains("MRC-v0.0.15-win-x64.zip", StringComparison.Ordinal), "Checksum authority does not name the v0.0.15 candidate ZIP.");
    }
