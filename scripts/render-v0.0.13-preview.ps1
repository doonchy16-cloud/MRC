[CmdletBinding()]
param(
    [string]$OutputDir = (Join-Path (Split-Path $PSScriptRoot -Parent) 'artifacts\v0.0.13')
)
...
function Invoke-MrcPreview {
    ...
    try {
        ...
        if ($LASTEXITCODE -ne 0) {
            throw "v0.0.13 preview renderer failed with exit code $LASTEXITCODE for ${Width}x${Height}."
        }
    }
    finally {
        Pop-Location
